...
public class RankSortMapper extends
	...
	public void map(LongWritable key, Text values, Context context) {
		...
		temp = source_title.toString();
		temp = "source_page is: "+ temp + " ; ";
		temp = temp + "target_pages are: "+ target_titles.toString();
		...
		if (tokenizer.hasMoreTokens()) {
			try {
				// fetch target_titles
				target_titles = new Text(tokenizer.nextToken().trim());
				String temp = source_title.toString();
				temp = "source_page is: "+ temp + " ; ";
				temp = temp + "target_pages are: "+ target_titles.toString();
				
				output_value = new Text(temp);
				
				context.write(pagerank, output_value);			
